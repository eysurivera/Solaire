...
if (strlen($_SESSION['hbmsaid']==0)) {
  ...
  header('location:logout.php');
	...
	if (isset($_GET['deleteid']))
	{
		...
		$sql = "delete from tblcontact where ID=:eid";
		$query = $dbh->prepare($sql);
		$query->bindParam(':eid', $eid, PDO::PARAM_STR);
        $query->execute();

		echo '<script>alert("Enquiry has been deleted.")</script>';

        echo "<script>window.location.href ='read-enquiry.php'</script>";
...
<title>Hotel Dy Grande | Read Enquiry</title>
...
{               ?>
                                    ...
                                    <tr>
										...
										<a href="view-enquiry.php?viewid=<?php echo htmlentities ($row->ID);?>">View Details</a>
										&nbsp
										 &nbsp
										 &nbsp
										 <a href='read-enquiry.php?deleteid=<?php  echo htmlentities($row->ID);?>'>
    										<button type="submit" class="btn btn-danger" name="delete" onclick="return confirm('Delete this Enquiry?')">Delete</button>
