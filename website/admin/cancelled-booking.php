  ...
  header('location:logout.php');
	...
	if(isset($_POST['stud_delete_multiple_btn']))
	{
		...
		if ($all_id != NULL){
			...
			$sql = "DELETE FROM tblbooking WHERE BookingNumber IN ($extract_id)";
			$query = $dbh->prepare($sql);
			$query->execute();
	
			echo '<script>alert("Multiple Booking has been deleted.")</script>';
		}
		else{
			echo '<script>alert("Please select booking to delete.")</script>';
		}
						...
						<div class="forms">
							...
							<div class="form-grids widget-shadow" data-example-id="basic-forms"> 
								...
								<div class="form-body">
										 ...
										 <form action="cancelled-booking.php" method="POST">
                                ...
                                <thead>
                                    <tr>
										...
										<button type="submit" name="stud_delete_multiple_btn" class="btn btn-danger" onclick="return confirm('Delete selected bookings?')">Delete Selected</button>
...
{               ?>
                                    ...
                                    <tr>
										...
										<td style="width:10px; text-align: center;">
                                        	<input type="checkbox" name="stud_delete_id[]" value="<?php  echo htmlentities($row->BookingNumber);?>">
                                   		</td>
...
<?php } ?> 
										 ...
										 <a href="view-booking-detail.php?bookingid=<?php echo htmlentities ($row->BookingNumber);?>"><i class="fa fa-eye" aria-hidden="true"></i></a>
										</td>
