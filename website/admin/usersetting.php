										...
										//	$ps = $row['pass'];
											if($id % 2 ==0 )
											{
												echo"<tr class='gradeC'>
													<td>".$id."</td>
													<td>".$us."</td>
                                                    <td><img src=' $pc' alt='Profile Picture' width='50'></td>

												
													
                                                                                                    <td>
                                                    <button class='btn btn-primary btn' data-toggle='modal' data-target='#myModal' data-id='$id'>
                                                        Update
                                                    </button>
                                                </td>
...
if (isset($_POST['up'])) {
    ...
    if ($_FILES['profile_pic']['name']) {
        $target_dir = "../images/avatar/"; // Corrected path
        $target_file = $target_dir . basename($_FILES["profile_pic"]["name"]);
        move_uploaded_file($_FILES["profile_pic"]["tmp_name"], $target_file);
        $updatePic = ", `profile_pic`='$target_file'";
    } else {
        $updatePic = ""; // No new profile picture selected
    }

    $upsql = "UPDATE `login` SET `usname`='$usname', `pass`='$passwr' $updatePic WHERE id = '$id'";
